...
namespace TempLat
{
  ...
  public:
    ...
    FFTWTranspositionFlags(const FFTLayoutStruct<NDim> &layout)
    {
      ...
      mFlagC2R = (transposedC ? FFTWTRANSPOSITIONFLAGS_TRANSPOSED_IN : 0) |
                 (transposedR ? FFTWTRANSPOSITIONFLAGS_TRANSPOSED_OUT : 0);

      mFlagR2C = (transposedC ? FFTWTRANSPOSITIONFLAGS_TRANSPOSED_OUT : 0) |
    ...
    friend std::ostream &operator<<(std::ostream &ostream, const FFTWTranspositionFlags &tr)
    {
      ostream << "C2R : "
              << ((tr.mFlagC2R & FFTWTRANSPOSITIONFLAGS_TRANSPOSED_IN)    ? " transposed in"
                  : (tr.mFlagC2R & FFTWTRANSPOSITIONFLAGS_TRANSPOSED_OUT) ? "transposed out"
                                                                          : "not transposed")
              << ".\n";
      ostream << "R2C : "
              << ((tr.mFlagR2C & FFTWTRANSPOSITIONFLAGS_TRANSPOSED_IN)    ? " transposed in"
                  : (tr.mFlagR2C & FFTWTRANSPOSITIONFLAGS_TRANSPOSED_OUT) ? "transposed out"
                                                                          : "not transposed")
              << ".\n";
      return ostream;
    }
