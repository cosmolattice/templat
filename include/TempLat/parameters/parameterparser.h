...
namespace TempLat
{
  ...
  public:
    ...
    template <class S> ParameterGetter<S> get(const std::string &name, ParameterImportance importance = Normal)
    {
      return this->get<S>(name, true, {S()}, false, Tag<1>(), importance)[0];
    }

    template <class S> ParameterGetter<S> get(const std::string &name, S def, ParameterImportance importance = Normal)
    {
    ...
    template <class S> ParameterGetter<S> getOverride(const std::string &name, ParameterImportance importance = Normal)
    {
    ...
    template <class S, int N>
    MultipleParameterGetter<S> getOverride(const std::string &name, std::vector<S> def,
    ...
    void activateOverride() { mOverride = true; }

    void insert(const std::string &str)
    { // Insert value of the current PairMaker in the unordered_map. If already exist, skip it.
      pairMaker(str);
      ...
      if (params.count(pairMaker.getKey()) == 0) {
        ...
        if (params[pairMaker.getKey()] == "")
          throw(
              ParameterParserIsEmpty("Parameter " + pairMaker.getKey() +
                                     " is empty. Abort. If it is an optional parameter, remove it or comment it out."));
      } else {
        if (mVerbose)
          say << "Parameter " + pairMaker.getKey() +
                     " already assigned, skipped. The last value of this parameter which is skipped will be cached to "
                     "have the potential of overriding it.";
        paramsOverride[pairMaker.getKey()] = pairMaker.getValue(); // Store last of the extra value here.
      }
    }

    void erase(const std::string &key) { params.erase(key); }
      ...
      for (const auto &i : par.params) {
        if (par.paramsImportance.count(i.first) > 0) {
          ...
          if (par.paramsImportance.at(i.first) == Important)
            importantParams << std::right << std::setw(20) << i.first << "       " << std::setw(20) << std::left
                            << second << std::endl;
          if (par.paramsImportance.at(i.first) == Normal)
            otherParams << std::right << std::setw(20) << i.first << "       " << std::setw(20) << std::left << second
                        << std::endl;
        }
      }
  ...
  private:
    ...
    std::unordered_map<std::string, std::string>
        paramsOverriden; // This is the list of parameters which have actually been overriden
    ...
    template <class S, int N>
    MultipleParameterGetter<S> get(const std::string &name, const bool &mandatory, std::vector<S> def, bool override,
                                   Tag<N>, ParameterImportance importance = Normal)
    { // give the templated integer as a tag so we can specialise for N=0.
      ...
      if ((!override || paramsOverride.count(name) == 0) && params.count(name) > 0) {
        ...
        if (N > 0 && mandatory && ret.size() != N)
          throw(ParameterParserMismatchSizes("Parameter " + name +
                                             " was provided with an unexpected number of arguments. Abort."));
        if (N > 0 && !mandatory && ret.size() > N)
          throw(ParameterParserMismatchSizes("Parameter " + name + " was provided with too many arguments. Abort."));
        ...
        throw(ParameterParserMismatchDefaultSizes(
            "Optional parameter " + name + " was not provided with the correct number of default values. Abort."));
      // else{
      if (ret.size() != N) {
