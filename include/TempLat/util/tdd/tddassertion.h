...
namespace TempLat
{
  ...
  public:
                    ...
                    std::string description = std::string())
    {
      ...
      SayComplete(fName, line) << (result ? ColouredOK() : ColouredERROR()) << " " << std::string(stringifiedExpression)
                               << " " << description << "\n";
    }

    void ASSERT(const char *stringifiedExpression, const char *fName, int line, bool result,
                std::string description = std::string())
    {
      results->emplace_back(result, stringifiedExpression);
      RecordSuccess(result);
    ...
    {
      stream << (ass.badScore > 0 ? ColouredERROR() : ColouredOK()) << " Passed " << ass.goodScore << " and failed "
             << ass.badScore << " tests.";
      return stream;
    }
  ...
  private:
    ...
    void RecordSuccess(const bool &success) { success ? ++goodScore : ++badScore; }
