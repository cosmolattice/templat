...
namespace TempLat
{
  ...
  template <int Start, int End> struct TagListHelper;

  template <int Start, int End> using MakeSeqImplTagList_t = typename TagListHelper<Start, End>::type;

  template <int N> using MakeSeqTagList_t = typename TagListHelper<0, N - 1>::type;

  template <int... I1s, int... I2s> struct Concat<TagListBase<I1s...>, TagListBase<I2s...>> {
    using type = TagListBase<I1s..., I2s...>;
  };

  template <int Start, int End> struct TagListHelper {
    using type =
        Concat_t<MakeSeqImplTagList_t<Start, (Start + End) / 2>, MakeSeqImplTagList_t<(Start + End) / 2 + 1, End>>;
  };

  // break condition:
  template <int I> struct TagListHelper<I, I> {
    using type = TagListBase<I>;
  };

  template <int START, int END> using TagList = typename TagListHelper<START, END>::type;
