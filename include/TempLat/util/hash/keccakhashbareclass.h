...
namespace TempLat
{
  ...
  public:
    ...
    std::string toString() const
    {
      ...
      for (unsigned int i = 0; i < hash.size(); ++i)
        snprintf(theHash.data() + 2 * i, 3, "%02x", hash[i]);
      theHash.back() = '\0';
      /* -1: we remove the trailing '\0', which is not by default present in a std::string. Relevant for test
       * comparisons. */
  ...
  private:
    ...
    {
      if (hasBeenUsed)
        throw KeccakHashBareClassReuseException(
            "A KeccakHashBareClass can only compute once, and then has a permanent state. Create a new one.");
      hasBeenUsed = true;
      if (endian.isLittle()) {
