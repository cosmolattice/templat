...
namespace TempLat
{
  ...
  //        say << dim;
          ...
          auto CovPlus = total(MakeArray(i,1,dim,
              binary_fold( [](auto x, auto y){return x*y;}, MakeArray(j,0,size-2,list.getComp(j).getComp(i-1_c)),[](auto
  x){return x;},shift(scalar,i))
          ...
          auto CovMinus = total(MakeArray(i,1,dim,
                                         binary_fold( [](auto x, auto y){return x*y;},
  MakeArray(j,0,size-2,dagger(shift(list.getComp(j).getComp(i-1_c),-i))),[](auto x){return x;},shift(scalar,-i))
                               ));
