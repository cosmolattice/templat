...
namespace TempLat
{
  ...
  void SU2SubtractTester::Test(TDDAssertion &tdd)
  {
    auto toolBox = MemoryToolBox<3>::makeShared(16, 1);
    SU2Field<float, 3> su2_1("testSU2_1", toolBox);
    SU2Field<float, 3> su2_2("testSU2_2", toolBox);
    ...
    for (size_t i = 0; i < f1view.extent(0); ++i)
      for (size_t j = 0; j < f1view.extent(1); ++j)
        for (size_t k = 0; k < f1view.extent(2); ++k) {
          all_true = all_true && (f1view(i, j, k) == 0.5f);
          all_true = all_true && (f2view(i, j, k) == -2.5f);
          all_true = all_true && (f3view(i, j, k) == 2.5f);
...
namespace
{
  TempLat::TDDContainer<TempLat::SU2SubtractTester> test;
}
