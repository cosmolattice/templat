...
namespace TempLat
{
  ...
  void SU2SumTester::Test(TDDAssertion &tdd)
  {
    ...
    for (size_t i = 0; i < f1view.extent(0); ++i)
      for (size_t j = 0; j < f1view.extent(1); ++j)
        for (size_t k = 0; k < f1view.extent(2); ++k) {
          all_true = all_true && (f1view(i, j, k) == 1.5f);
          all_true = all_true && (f2view(i, j, k) == -0.5f);
          all_true = all_true && (f3view(i, j, k) == 1.5f);
...
namespace
{
  TempLat::TDDContainer<TempLat::SU2SumTester> test;
}
